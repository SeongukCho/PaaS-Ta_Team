...
public class EncryptUtil {
     ...
     * 암호화 된 값만 보고 일반적인 비밀버호에 대한 값을 쉽게 예측이 가능함
     * 따라서, 암호화 할때 암호화되는 값에 추가적인 문자열을 붙여서
     * 함께 암호화를 진행함
     */
    ...
    // 초기 벡터(16바이트의 크기를 가지며, 16바이트 단위로 암호화시
    // 암호화 할 총 길이가 16바이트가 되지 못하면 뒤에 추가하는 바이트)
    ...
    public static String encHashSHA256(String str) throws Exception {
        ...
        try {
            ...
            res = sb.toString();
        } catch (NoSuchAlgorithmException e) { // 자바에서 제공하는 알고리즘이 아닌경우, 에러 발생
            e.printStackTrace();
